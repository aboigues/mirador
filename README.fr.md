...
> Le préfixe `GITHUB_` est réservé par GitHub Actions. D'où `APP_ID` / `APP_PRIVATE_KEY` côté GitHub, que `deploy-config.yml` remappe en `GITHUB_APP_ID` / `GITHUB_APP_PRIVATE_KEY` côté Scaleway. De même, Scaleway Functions réserve le préfixe `SCW_` : les variables posées sur les fonctions utilisent `SCALEWAY_`.
