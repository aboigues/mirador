...
- [x] T001 Ajouter l'extra `mcp = ["mcp>=2.2.0"]`, l'ajouter à l'extra `dev`, et le point
  d'entrée `mirador-mcp = "src.interface_mcp.serveur:main"` dans `pyproject.toml`
- [x] T002 Créer le paquet `src/interface_mcp/__init__.py`

---

## Phase 2 : Fondations (bloquant pour tous les scénarios)

- [x] T003 [P] Test : `grouper_anomalies` et la table des statuts dans
  `tests/unitaire/test_vue_anomalies.py` (regroupement par corrélation, statut inféré du
  dernier événement, ordre conservé)
- [x] T004 Déplacer `_grouper_anomalies` et `_STATUT_PAR_EVENEMENT` de
...
- [x] T005 [P] Test : `ClientBucketLectureFichier` dans
...
- [x] T006 Implémenter `src/infrastructure/stockage/lecture_fichier.py`
- [x] T007 [P] Modèles de sortie Pydantic dans `src/interface_mcp/modeles.py`
  (data-model.md)
- [x] T008 Squelette `src/interface_mcp/serveur.py` : `creer_serveur(config, bucket)`,
...
- [x] T009 [P] Test d'intégration `tests/integration/test_serveur_mcp.py::test_liste_exacte_des_outils`
...
- [x] T010 [P] [US1] Tests unitaires dans `tests/unitaire/test_outils_mcp.py` :
...
- [x] T011 [P] [US1] Test d'intégration CS-002 dans `tests/integration/test_serveur_mcp.py` :
  mêmes anomalies via le client MCP et via l'API de consultation sur le même journal
- [x] T012 [US1] Implémenter `lister_depots` et `lister_anomalies` dans
...
- [x] T013 [P] [US2] Tests unitaires `historique_anomalie` : ordre chronologique,
  acteurs, `details` et `delivery_id` exclus, UUID invalide → `ToolError`, UUID inconnu →
  `ToolError`
- [x] T014 [US2] Implémenter `historique_anomalie` dans `outils.py` et l'enregistrer

---

## Phase 5 : US3 — Intégrité du journal (P2)

- [x] T015 [P] [US3] Tests `tests/unitaire/test_integrite.py` avec un vrai `WriterAudit` :
...
- [x] T016 [US3] Implémenter `src/infrastructure/stockage/integrite.py`
...
- [x] T017 [P] [US4] Tests unitaires `classer_echec` : règle active reconnue → LOW + nom
...
- [x] T018 [US4] Implémenter `classer_echec` dans `outils.py` (message `workflow_run`
...
- [x] T019 Test `tests/integration/test_serveur_mcp_stdio.py` : serveur lancé en
  ...
  3 s (CS-006)
- [x] T020 [P] Évaluation `tests/eval_outils_mcp.py` (10 questions, seuil 9/10,
  `claude-haiku-4-5`) et workflow `.github/workflows/eval-outils-mcp.yml`
  (`workflow_dispatch`, secret `ANTHROPIC_API_KEY`) — écrits ; **première exécution à faire** après push (pas de clé API en local)
- [x] T021 [P] Documentation : section « Serveur MCP » dans `README.md` (anglais, noms
  d'outils expliqués) et `README.fr.md`
- [x] T022 Lancer toute la suite (`pytest tests/unitaire tests/integration`) et la
